...
class HMMDetector(BaseSegmenter):
    ...
    _DEFAULT_EMISSION_FUNCS = [
        (norm.pdf, {"loc": 0.0, "scale": 1.0}),
        (norm.pdf, {"loc": 1.0, "scale": 1.0}),
    ]
    _DEFAULT_TRANSITION_MAT = np.array([[0.9, 0.1], [0.1, 0.9]])

    def __init__(
        self,
        emission_funcs: list | None = None,
        ...
        self.emission_funcs = (
            emission_funcs
            if emission_funcs is not None
            else list(self._DEFAULT_EMISSION_FUNCS)
        )
        self.transition_prob_mat = (
            transition_prob_mat
            if transition_prob_mat is not None
